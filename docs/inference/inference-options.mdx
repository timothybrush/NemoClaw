...
<Warning>
  Reasoning mode validates only `/v1/chat/completions`; it does not verify tool calling or
  streaming. Enable it only when the endpoint supports the capabilities your agent needs.
</Warning>
...
NemoClaw tests streaming events during onboarding and uses chat completions without probing the Responses API.
